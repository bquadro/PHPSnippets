...
CKEDITOR.plugins.add('jqueryspellchecker', {
  ...
  create: function() {
    ...
    jQuery(this.editorWindow)
    .on('scroll.spellchecker', jQuery.proxy(function scroll(){
  ...
  createSpellchecker: function() {
    ...
    t.config.getText = function() {
      return jQuery('<div />').append(t.editor.getData()).text();
    };

    t.spellchecker = new jQuery.SpellChecker(t.editor.document.$.body, this.config);
  ...
  positionSuggestBox: function() {
    ...
    return function() {
      ...
      var p1 = jQuery(ed.container.$).find('iframe').offset();
      var p2 = jQuery(ed.container.$).offset();
      var p3 = jQuery(word).offset();

      var left = p3.left + p2.left;
      var top = p3.top + p2.top + (p1.top - p2.top) + word.offsetHeight;

      top -= jQuery(t.editorWindow).scrollTop();
